...
export function hrefForSearchResult(result: SearchResult): string {
  if (result.type === "artist") return `/design-lab/live/artist/${result.discogs_id}`;
  if (result.type === "label") return `/design-lab/live/label/${result.discogs_id}`;
  if (result.type === "master") return `/design-lab/live/release/${result.discogs_id}`;
  return `/design-lab/live/version/${result.discogs_id}`;
}

export function hrefForTraversalLink(link: TraversalLink): string {
  if (link.type === "artist") return `/design-lab/live/artist/${link.discogs_id}`;
  if (link.type === "label") return `/design-lab/live/label/${link.discogs_id}`;
  if (link.type === "master") return `/design-lab/live/release/${link.discogs_id}`;
  return `/design-lab/live/version/${link.discogs_id}`;
}
